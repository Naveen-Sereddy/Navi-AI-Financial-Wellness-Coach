...
(() => {
  ...
  function parseDcDocument(doc) {
    const dc = doc.querySelector("x-app");
    if (!dc) return null;
    const scriptEl = doc.querySelector("script[data-app-script]");
  ...
  function parseDcText(src) {
    const openMatch = /<x-app(?:\s[^>]*)?>/.exec(src);
    if (!openMatch) return null;
    const close = src.lastIndexOf("</x-app>");
  ...
  function evalComponent(src) {
    ...
    return fn(NaviBase, NaviBase, getReact());
  }
  ...
  function createComponentFactory(registry, ensureFetched) {
    ...
    class StreamableComponent extends React.Component {
      ...
      __reconcileLogic() {
        ...
        if (Next === Cur || !Next && Cur === NaviBase || Next === this.__failedLogic && r.ver === this.__failedVer && shallowEqual(this.__userProps(), this.__failedUserProps)) {
          return;
        }
  ...
  function createRuntime(doc = document) {
      ...
      fetch(url).then((res) => {
        ...
        if (!parsed) {
          console.error(
            ...
            url,
            "has no <x-app> block \u2014 not a Component."
          );
          return;
        }
    ...
    function updateJs(name, src) {
      ...
      try {
        const Cls = evalComponent(src);
        if (r.jsSeq !== seq) return;
        if (typeof Cls !== "function") {
          r.logicError = name + ".html: <script data-app-script> must define `class Component extends NaviBase`";
        } else {
          r.logicError = null;
          r.Logic = Cls;
        }
  ...
  function hideRawTemplate() {
    ...
    s.textContent = "x-app{display:none!important}";
    document.head.appendChild(s);
  }
  ...
  function init() {
    ...
    const api = {
      ...
      // `NaviBase` is the documented base class name; `NaviBase` is the
      // implementation alias kept for any project that already references it.
      NaviBase: runtime.NaviBase,
      NaviBase: runtime.NaviBase
    };
